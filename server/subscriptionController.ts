...
async function ensureStripeCustomer(user: any): Promise<string> {
  if (user.stripeCustomerId) {
    console.log(`♻️  Using existing Stripe customer ID: ${user.stripeCustomerId}`);
 ...
 */
async function createDefaultSubscription(user: any, customerId: string): Promise<any> {
  console.log(`🔄 Creating default Starter subscription for user ${user.id}...`);
  ...
  console.log(`✅ Created default Starter subscription for user ${user.id}: ${subscription.id}`);
...
export async function getBillingCycleWordUsageAnalytics(userId: number) {
  try {
    ...
    console.log(`📅 Billing cycle: ${subscriptionStart.toISOString()} to ${subscriptionEndExclusive.toISOString()}`);
    ...
    console.log(`📊 Word usage summary: ${usageReport.totalWordCount} words across ${usageReport.recordingCount} recordings`);

    // 4. Calculate comprehensive analytics
    const wordLimit = subscriptionData.wordLimit;
    const usagePercentage = Math.round((usageReport.totalWordCount / wordLimit) * 100);
    ...
    const now = new Date();
    const msRemaining = subscriptionData.currentPeriodEnd.getTime() - now.getTime();
    ...
    const analytics = {
      ...
      analytics: {
        ...
        billingCycleProgress: {
          ...
          percentComplete: Math.round(((now.getTime() - subscriptionStart.getTime()) / (subscriptionData.currentPeriodEnd.getTime() - subscriptionStart.getTime())) * 100)
        }
      }
    };

    console.log(`✅ COMPREHENSIVE BILLING ANALYTICS COMPLETE`);
    console.log(`📈 Usage: ${usageReport.totalWordCount}/${wordLimit} words (${usagePercentage}%)`);
