...
export default function Training() {
  ...
  const totalSituations = chapters?.reduce((total: number, chapter: any) => 
    total + chapter.modules.reduce((moduleTotal: number, module: any) => 
      moduleTotal + module.scenarios.length, 0), 0) || 0;
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Master essential leadership skills through interactive scenarios and personalized feedback.
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between text-sm">
              ...
              <span>{completedSituations} of {totalSituations}</span>
            </div>
        ...
        <div className="space-y-4">
          ...
          {chapters?.map((chapter: any, index: number) => {
            const chapterProgress = chapter.modules.reduce((total: number, module: any) => {
              const moduleCompleted = userProgress?.filter((p: any) => 
                p.chapterId === chapter.id && p.moduleId === module.id
              ).length || 0;
              return total + moduleCompleted;
            }, 0);
            
            const chapterTotal = chapter.modules.reduce((total: number, module: any) => 
              total + module.scenarios.length, 0);
                ...
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <Progress value={chapterPercentage} className="h-1 w-20 mt-1" />
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {chapter.modules.map((module: any) => {
                      ...
                      return (
                        <div key={module.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-md">
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            {isModuleComplete ? (
                              ...
                              <Link href={`/training/chapter/${chapter.id}/module/${module.id}`}>
